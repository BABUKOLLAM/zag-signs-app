...
interface StatCardProps {
  ...
  trendUp?: boolean; icon: ComponentType<{ size?: number; className?: string }>; gradient: string;
}
...
export default function Dashboard() {
  ...
  const { data: s, loading, error, refetch } = useApi<DashboardStats>("/dashboard");
      ...
      <div className="p-4 md:p-6 space-y-6">
        ...
        <div className="rounded-2xl p-5 flex items-center justify-between overflow-hidden relative"
          ...
          <div className="relative">
            ...
            <h2 className="text-white text-xl font-bold mt-0.5">Good day, {userName.split(" ")[0]}</h2>
            <p className="text-indigo-200 text-sm mt-1">
              {s ? (
                <>You have <span className="text-white font-semibold">{s.totalLeads} leads</span> and <span className="text-white font-semibold">{s.openComplaints} open complaints</span>.</>
              ) : "Welcome to ZAG SIGNS ERP."}
            </p>
          </div>
          <div className="relative items-center gap-3 hidden lg:flex">
            {s && [
              { label: "Pipeline Value", value: fmtShort(s.pipelineValue) },
              { label: "Active Leads", value: s.totalLeads },
              { label: "Open Orders", value: s.ordersThisMonth },
            ].map((item) => (
              <div key={item.label} className="bg-white/15 backdrop-blur rounded-xl px-4 py-3 text-center">
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              <StatCard title="Total Leads" value={s.totalLeads}
                sub={`+${s.newLeadsThisMonth} this month`}
                icon={Users} gradient="bg-gradient-to-br from-blue-500 to-cyan-500" />
              <StatCard title="Pipeline Value" value={fmtShort(s.pipelineValue)}
                sub="Active leads" icon={TrendingUp} gradient="bg-gradient-to-br from-violet-500 to-purple-600" />
              <StatCard title="Orders / Month" value={s.ordersThisMonth}
                sub={new Date().toLocaleString("en-IN", { month: "long", year: "numeric" })}
                icon={ShoppingCart} gradient="bg-gradient-to-br from-emerald-500 to-teal-600" />
              <StatCard title="Revenue MTD" value={fmtShort(s.revenueThisMonth)}
                sub="Invoiced & collected" icon={Wallet} gradient="bg-gradient-to-br from-indigo-500 to-blue-600" />
              <StatCard title="Collection"
                value={`${Math.round((s.collectionAchieved / Math.max(s.collectionTarget, 1)) * 100)}%`}
                sub={`${fmtShort(s.collectionAchieved)} of ${fmtShort(s.collectionTarget)}`}
                icon={Target} gradient="bg-gradient-to-br from-amber-500 to-orange-500" />
              <StatCard title="Complaints" value={s.openComplaints}
                sub="Pending resolution" icon={AlertCircle} gradient="bg-gradient-to-br from-red-500 to-rose-600" />
            </div>
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 lg:col-span-3">
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <AreaChart data={s.monthlyRevenue}>
                    ...
                    <YAxis tickFormatter={(v) => `${(Number(v) / 100000).toFixed(0)}L`} tick={{ fontSize: 11, fill: "#94A3B8" }} axisLine={false} tickLine={false} />
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 lg:col-span-2">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">This month&apos;s revenue</p>
                  </div>
                ...
                <div className="space-y-3">
                  {[...s.branchPerformance]
                    ...
                    .map((b, i) => {
                      const max = Math.max(...s.branchPerformance.map((x) => x.revenue), 1);
                        ...
                        <div key={b.branch}>
                          <div className="flex items-center justify-between mb-1">
                            <div className="flex items-center gap-2">
                              <span className={`text-xs font-bold w-5 h-5 rounded flex items-center justify-center text-white bg-gradient-to-br ${colors[i % 4]}`}>{i + 1}</span>
                          ...
                          <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                            <div className={`h-full rounded-full bg-gradient-to-r ${colors[i % 4]}`} style={{ width: `${Math.max(4, pct)}%` }} />
                          </div>
                        </div>
                      );
                    })}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-xs text-slate-400">{s.leadFunnel.reduce((t, f) => t + f.count, 0)} active leads</span>
                </div>
                <div className="space-y-2.5">
                  {s.leadFunnel.length === 0 ? (
                    <p className="text-sm text-slate-400 text-center py-6">No active leads in pipeline</p>
                  ) : (() => {
                    const maxCount = Math.max(...s.leadFunnel.map((f) => f.count), 1);
                    const colors = ["bg-indigo-500", "bg-violet-500", "bg-blue-500", "bg-cyan-500", "bg-teal-500", "bg-emerald-500"];
                    return s.leadFunnel.map((f, i) => (
                      <div key={f.stage} className="flex items-center gap-3">
                        <span className="text-xs text-slate-500 w-24 flex-shrink-0 font-medium">{f.stage}</span>
                        <div className="flex-1 bg-slate-100 rounded-full h-6 overflow-hidden">
                          <div
                            className={`h-full rounded-full flex items-center justify-end pr-2.5 ${colors[i % 6]}`}
                            style={{ width: `${Math.max(8, Math.min(100, (f.count / maxCount) * 100))}%` }}
                          >
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
                {(() => {
                  const collPct = Math.round((s.collectionAchieved / Math.max(s.collectionTarget, 1)) * 100);
                  return (
                    <>
                      <div className="flex items-center justify-between mb-4">
                        <div>
                          <h3 className="text-sm font-semibold text-slate-800">Collection Tracker</h3>
                          <p className="text-xs text-slate-400 mt-0.5">This month&apos;s target progress</p>
                        </div>
                      ...
                      <div className="flex items-center gap-6 mb-5">
                        ...
                        <div className="space-y-2">
                          ...
                          <div>
                            ...
                            <p className="text-sm font-semibold text-amber-600">{fmt(Math.max(0, s.collectionTarget - s.collectionAchieved))}</p>
                          </div>
                        </div>
                      </div>
